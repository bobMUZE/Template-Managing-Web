...
from tpllist.models import Customer, Sites, Templates, Users
...
class CustomerSiteLV(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['sites'] = Sites.objects.filter(customer_id=self.kwargs['pk'], is_root=1)
        context['customer_id'] = self.kwargs['pk']
        context['curr_customer'] = Customer.objects.filter(customer_id=self.kwargs['pk'])
        # context['customers'] =Customer.objects.all()
        return context
...
class CustomerSiteDV(DetailView):
    ...
    def get_context_data(self, **kwargs):
        current_path = Sites.objects.get(site_id=self.kwargs['pk'])
        ...
        context['subpages'] = Sites.objects.filter(site_url__regex=path_regex)
...
class CustomerTplCV(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['curr_customer'] = Customer.objects.filter(customer_id=self.kwargs['customer_id'])
        context['templates'] = Templates.objects.filter(site_id=self.kwargs['pk'])
        context['curr_site'] = Sites.objects.get(site_id=self.kwargs['pk'])
        return context
...
class CustomerPermissionLV(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['permissions'] = Users.objects.filter(customer_id=self.kwargs['pk'])
        context['customer_id'] = self.kwargs['pk']
        context['curr_customer'] = Customer.objects.filter(customer_id=self.kwargs['pk'])
        return context
